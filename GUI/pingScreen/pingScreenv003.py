"""
this is pingScreenv003.py

updates:
- fixed drawLine float/int issue by casting to int()
- ensured painter.end() is always called correctly
- no rotation, slice always points upward
- ripple/wave animation inside the slice
- uses glitch theme (white/cyan/magenta/red) for grid
...
class RadarWidget(QWidget):
    def __init__(self):
        super().__init__()
        self.distance_in = None  # latest ping measurement
        self.ripple_phase = 0    # drives the wave animation

        self._timer = QTimer(self)
        self._timer.timeout.connect(self._tick)
        self._timer.start(30)    # ~33 fps

    # drive ripple animation
    def _tick(self):
    ...
    def paintEvent(self, e):
        ...
        try:
            ...
            # build non-rotating pizza slice pointing upward (60° wide)
            start_angle = -90 - 30
            end_angle   = -90 + 30
            ...
            # draw glitch-theme grid arcs
            for i, col in enumerate(self._theme_grid_colors()):
                p.setPen(QPen(col, 2))
                r = int(radius * (0.25 + 0.15 * i))
                p.drawArc(cx - r, cy - r, 2 * r, 2 * r, 180 * 16, 180 * 16)

            # base color for slice
            base_color = self._distance_color()

            # apply clipping so ripples stay inside slice
            ...
            for yOffset in range(0, radius, 6):
                ...
                x1 = int(cx - ripple * 0.2)
                x2 = int(cx + ripple * 0.2)
                yy = int(cy - yOffset)

                # all ints now → no crash

...
class LEDWorker(QThread):
    ...
    Modes:
      - 'standby'      : continuous rainbow
      - 'green'        : solid green
      - 'yellow_pulse' : short yellow flash sequence, then auto-return to standby
    """
    ...
    def __init__(self, num_leds=5, parent=None):
        ...
        # connect external signals to internal mode setter
    ...
    def _set_mode(self, m: str):
        """
        Set current animation mode. This is read from the run() loop.
        """
        self._mode = m

    def _hue_to_rgb(self, h):
        """
        Simple HSV→RGB-ish mapping for rainbow.
        """
        h = h % 360
    ...
    def _rainbow_step(self, idx: int):
        """
        One frame of the rainbow standby animation.
        """
    ...
    def _set_all(self, r, g, b):
        """
        Set all LEDs on both strips to the same color.
        """
        for i in range(self.strip0.led_count):
            self.strip0.RGBto3Bytes(i, r, g, b)
            self.strip1.RGBto3Bytes(i, r, g, b)
        ...
        while not self._stop:
            try:
                ...
                if mode == "standby":
                    ...
                    # solid green, but keep loop lightweight
                    self._set_all(0, 150, 0)
                    self.msleep(150)

                elif mode == "yellow_pulse":
                    # SHORT, SELF-LIMITED pulse sequence
                    # after this, automatically go back to standby
                    for _ in range(2):
                        ...
                        self.msleep(120)
                    # IMPORTANT: do NOT stay in yellow_pulse forever
                    # automatically revert to standby so the thread never gets "stuck"
                    self._mode = "standby"

                else:
                    # unknown mode: just idle briefly
                    self.msleep(100)

            except Exception:
                # basic fault-tolerance: don't die on one bad SPI call
                self.msleep(200)

        # on stop, turn everything off
        try:
            self.strip0.LED_OFF_ALL()
            self.strip1.LED_OFF_ALL()
        except Exception:

...
#this works but i think has leakage issues 
...
# LED worker for all the work the leds are doing.
class LEDWorker(QThread):
    ...
    def __init__(self, num_leds=5, parent=None):
        ...
        # Modes
        self._steady_mode = "standby"   # rainbow or green
        self._action_mode = None         # yellow or pink
        self._stop = False

        # Connect signals
        self.to_standby.connect(lambda: self._set_steady("standby"))
        self.to_green.connect(lambda: self._set_steady("green"))
        self.to_yellow_pulse.connect(lambda: self._start_yellow())
        self.to_pink_flash.connect(lambda: self._set_action("pink_flash"))

    def stop(self):
        self._stop = True

    def reset(self):
        """Reset for next USB session"""
        self._action_mode = None
        self._steady_mode = "standby"

    def _set_steady(self, mode):
        self._steady_mode = mode

    def _set_action(self, mode):
        self._action_mode = mode

    def _start_yellow(self):
        self._action_mode = "yellow_pulse"
    ...
    def run(self):
        ...
        while not self._stop:
            try:
                if self._action_mode == "yellow_pulse":
                        for i in range(self.strip0.led_count):
                ...
                elif self._action_mode == "pink_flash":
                    for _ in range(4):
                        for i in range(self.strip0.led_count):
                            self.strip0.RGBto3Bytes(i, 255, 20, 147)
                            self.strip1.RGBto3Bytes(i, 255, 20, 147)
                        self.strip0.LED_show()
                        self.strip1.LED_show()
                        self.msleep(180)
                        self.strip0.LED_OFF_ALL()
                    ...
                    # IMPORTANT — reset back to normal mode
                    self._action_mode = None
                    self._steady_mode = "standby"

                else:
                    # steady modes
                    if self._steady_mode == "standby":
                        self._rainbow_step(idx)
                        self.msleep(50)
                    elif self._steady_mode == "green":
                        for i in range(self.strip0.led_count):
...
class ShipScreen(QWidget):
    def __init__(self):
        ...
        self._found = set()
        self._all_done_fired = False      # <-- FIX 1: ensure tracking for repeated runs
    ...
    def set_manifest_codes(self, codes):
        ...
        self._found.clear()
        self._all_done_fired = False       # <-- FIX 2
    ...
    def _on_ready(self, dist_in, label):
        ...
        # reset all_done flag for safety
        self._all_done_fired = False

        # start two barcode workers fresh
        for idx in range(2):
        ...
        # first decode pulse (reset every run)
    ...
    def _on_all_done(self):
        ...
        # flash LEDs pink
        if self._leds:
            self._leds.to_pink_flash.emit()  # async pink flash

        #reset barcode workers for next run
        self._barcode_worker = None
        self._barcode_worker2 = None
    ...
    def _wait_for_usb_removal(self):
        usb_present = False
        roots = ["/media/jetson", "/media", "/run/media"]
        ...
        for root in roots:
            ...
            for sub in os.listdir(root):
                ...
                # Jetson-specific: mount points populated only when a USB device is inserted
                if os.path.ismount(full) and os.listdir(full):
                    usb_present = True
                    break
        ...
        if not usb_present:
            ...
            QTimer.singleShot(1000, self._wait_for_usb_removal)
    ...
    def _return_to_welcome(self):
        ...
        # Go back to welcome screen on main stacked widget
        try:
            self._main.setCurrentIndex(0)
        except Exception as e:
            self._log(f"Failed to return to welcome: {e}")

    # ---------------- PING WORKERS ----------------
    def on_attach(self, main_window):
        self._leds = getattr(main_window, "leds", None)
        self._main = main_window          # Allow return to welcome
        self.welcome = main_window.welcome
...
class MainWindow(QStackedWidget):
    def __init__(self):
        ...
        # setting the GUI window size we need to change this.(gui window size)

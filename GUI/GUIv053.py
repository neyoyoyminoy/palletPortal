...
class LEDWorker(QThread):
    ...
    def _set_steady(self, m):
        if self._completion_active:
            # once completion started, ignore new steady modes
            return
        self._steady_mode = m
        self._mode = m
            ...
            try:
                ...
                if mode == "standby":
                    ...
                    for _ in range(2):
                        ...
                        self.msleep(100)
                    # return to steady mode after pulse
...
class ShipScreen(QWidget):
    def __init__(self):
        ...
        self._start_time = None  # <-- track scan start time
    ...
    def set_current_usb_path(self, path):
        ...
        self._start_time = datetime.now()  # record start time
    ...
    def _return_to_welcome(self):
        ...
        if getattr(self, "_current_usb_path", None):
            ...
            # --- NEW: record order info in ViewOrderScreen ---
            end_time = datetime.now()
            scanned_count = len(self._found)
            trailer_number = os.path.basename(self._current_usb_path)
            if hasattr(mw, "view") and isinstance(mw.view, ViewOrderScreen):
                mw.view.add_order(
        ...
        if completed >= 3:
            try:
                mw.setCurrentIndex(3)  # go to ViewOrderScreen
            except Exception:
                pass
            return
...
class ViewOrderScreen(QWidget):
    def __init__(self):
        ...
        # List widget to show completed orders
        self.order_list = QListWidget()
        self.order_list.setSelectionMode(QListWidget.NoSelection)
        layout.addWidget(self.order_list)

        # Internal storage of orders
        # Each order is a dict with keys: start, end, scanned_count, archway, trailer
        self.orders = []

    def add_order(self, start_time, end_time, scanned_count, trailer_number):
        """Add a completed order record"""
    ...
    def _refresh_list(self):
        ...
        for o in self.orders:
            start_str = o["start"].strftime("%H:%M:%S")
            end_str = o["end"].strftime("%H:%M:%S")
            duration_str = str(o["duration"]).split(".")[0]  # hh:mm:ss
            text = (
                f"Trailer: {o['trailer']} | Archway: {o['archway']} | "
                f"Start: {start_str} | End: {end_str} | "
                f"Duration: {duration_str} | Scanned: {o['scanned_count']}"
            )

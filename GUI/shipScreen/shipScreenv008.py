...
layout goals:
- centered glitch title "SHIPMENT IN PROGRESS" (same class behavior as welcome screen)
...
from PyQt5.QtCore import Qt, QTimer, QRect  #qt core + timers + rect
...
# -------------------- glitch title widget (same logic as welcome screen) --------------------
class GlitchTitle(QWidget):
    def __init__(self, text="SHIPMENT IN PROGRESS", parent=None):
        ...
        self.font = QFont("Arial", 40, QFont.Bold)  #slightly smaller so it never clips
...
class PillProgressBar(QWidget):
    ...
    def minimumSizeHint(self):
        return self.parent().fontMetrics().boundingRect("Mg").height() * 12, 32
...
class ShipScreen(QWidget):
    def __init__(self, parent=None):
        ...
        # spacer so top of left bubble roughly aligns with top of right white panel
        ...
        # put progress bar roughly around middle of screen
        ...
        # style bubble with rounded corners matching right panel
        self.setStyleSheet(
            """
            QWidget {
        ...
        # match panel rounding with bubble
        extra_style = (
            self.styleSheet()
            + """
            QWidget#scanPanel {

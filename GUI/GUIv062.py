...
class BarcodeReaderWorker(QThread):
    ...
    def __init__(
        ...
        # store original manifest codes and build normalized LUT for resilient matching
        self._manifest_codes = list(manifest_codes or [])
        self._normalized_lut = {}  # normalized -> canonical manifest code
        for c in self._manifest_codes:
            if c is None:
                continue
            nc = self._normalize_code(c)
            if nc:
                self._normalized_lut[nc] = c

        # set of normalized codes we've already found (prevents re-add)
        self._found = set()
    ...
    def _normalize_code(s):
        """Normalize barcode string for matching: remove non-alnum, collapse whitespace, lower-case."""
        if s is None:
            return ""
        try:
            # ensure str, strip BOM/newlines, collapse whitespace
            s2 = str(s).strip()
            # remove common non-printable/trailing characters
            s2 = s2.replace("\ufeff", "")
            # collapse internal whitespace
            s2 = " ".join(s2.split())
            # remove everything except alnum (keeps letters+digits). Some barcodes include - or /; decide policy:
            # we remove punctuation so 'ABC-123' and 'ABC123' match. If you need punctuation preserved, relax this.
            import re
            s3 = re.sub(r"[^0-9A-Za-z]", "", s2)
            return s3.lower()
        except Exception:
        ...
        for (x1, y1, x2, y2) in rois:
            ...
            for r in res:
                try:
                    ...
                    val = None
                if val:
                    # keep original raw result for debug, but dedupe normalized value
                    norm = self._normalize_code(val)
                    if norm and norm not in {self._normalize_code(x) for x in out}:
                        out.append(val)
        return out
    ...
    def run(self):
        ...
        pipeline = None
        try:
            # Keep the SimpleManifestMatcher for backward compatibility, but rely primarily on normalized LUT.
            ...
            while not self._stop:
                ...
                if decoded:
                    ...
                    for raw_val in decoded:
                        # always emit raw decoded for debug/trace
                        try:
                            self.decoded.emit(raw_val)
                        except Exception:
                            pass

                        # try exact matcher first (keeps original behavior)
                        rec, score, method = matcher.match(raw_val)
                        if rec:
                            norm = self._normalize_code(rec)
                        else:
                            norm = self._normalize_code(raw_val)

                        # if normalized code maps to canonical manifest code, use it
                        canonical = self._normalized_lut.get(norm)
                        if canonical:
                            # avoid repeated handling of same logical code
                            if norm in self._found:
                                self.log.emit(f"{raw_val} matched but already recorded ({canonical})")
                            else:
                                self._found.add(norm)
                                self.matched.emit(canonical, 100, "normalized")
                                self.log.emit(f"{raw_val} → matched as {canonical}")
                        else:
                            # if SimpleManifestMatcher found earlier via exact, use that (handles subtle cases)
                            if rec:
                                nrec = self._normalize_code(rec)
                                if nrec not in self._found:
                                    self._found.add(nrec)
                                    self.matched.emit(rec, score or 100, method or "exact")
                                    self.log.emit(f"{raw_val} → matched as {rec}")
                                else:
                                    self.log.emit(f"{raw_val} matched as {rec} (already recorded)")
                            else:
                                # not found in manifest — log for debugging; may be noise or need different preprocessing
                                self.log.emit(f"{raw_val} is not part of shipment (normalized={norm})")
...
class ViewOrderScreen(QWidget):
    ...
    def keyPressEvent(self, event):
        """Handle X/C key to return to welcome screen via MainWindow"""
        if event.key() in (Qt.Key_X, Qt.Key_C):
            self.return_to_welcome.emit()  # parent MainWindow should switch screens

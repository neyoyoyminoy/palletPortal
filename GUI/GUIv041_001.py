...
class LEDWorker(QThread):
    ...
    def __init__(self, num_leds=5, parent=None):
        ...
        # Modes
        self._steady_mode = "standby"  # standby | green
        self._action_mode = None       # yellow_pulse | pink_flash
        self._stop = False

        # Signals → internal setters
        self.to_standby.connect(lambda: self._set_steady("standby"))
        self.to_green.connect(lambda: self._set_steady("green"))
        self.to_yellow_pulse.connect(lambda: self._set_action("yellow_pulse"))
    ...
    def reset_for_next_usb(self):
        """
        ShipScreen calls this AFTER a USB finishes.
        Fully resets LED state for the next USB cycle.
        """
    ...
    def run(self):
        ...
        while not self._stop:
            try:
                # Action mode overrides steady mode
                if self._action_mode == "yellow_pulse":

                    # Non-blocking repeated yellow flashes
                    for _ in range(2):
                        for i in range(self.strip0.led_count):
                    ...
                    # stays in yellow_pulse until overridden
                    self.msleep(600)
        ...
        # When thread stops → turn LEDs off
        try:
            self.strip0.LED_OFF_ALL()
            self.strip1.LED_OFF_ALL()
        except Exception:
